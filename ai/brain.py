...
请根据以上信息回答:"""
    ...
    def _llm_expand_query(self, query: str, max_variants: int = 4) -> List[str]:
        ...
        try:
            ...
            # Relaxed prompt: return plain keyword/phrase variants (no special prefixes)
            instruction = (
                "你的任务：为用户的原始查询生成最多 {max_variants} 个检索友好的查询变体，形式为简洁的关键词或短语，用于向向量检索系统扩展检索表达。\n"
                "要求：\n"
                "1) 只使用用户查询中明确出现或可直接推导的词语或短语，不要凭空添加实体或信息。\n"
                "2) 返回的每个变体应为简洁的自然语言短语或关键词，不要使用任何结构化前缀（例如不要返回类似【职位标题】: ... 的标签）。\n"
                "3) 每个变体应不超过 20 个中文字符或大约 8 个英文单词，避免重复。\n"
                "4) 严格只返回一个 JSON 数组文本（以 '[' 开始并以 ']' 结束），数组元素为字符串；响应中不得包含其他说明性文字或格式。\n"
                "示例：\n  - 输入：上海有哪些后端职位？\n  - 合法输出： [\"上海 后端\", \"后端 Java\"]\n\n"
            )
